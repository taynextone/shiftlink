...
export const updateNurseProfileSchema = z
  .object({
    ...
    birthPlace: z.string().trim().min(1).max(120).optional(),
    homeAddress: z.string().trim().min(1).max(300).optional(),
    nationality: z.string().trim().min(2).max(80).optional(),
    socialSecurityNumber: z.string().trim().min(5).max(40).optional(),
    taxId: z.string().trim().min(5).max(40).optional(),
    healthInsuranceName: z.string().trim().min(2).max(120).optional(),
