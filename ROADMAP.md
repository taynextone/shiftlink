...
- [done] Browser QA report CLI can parse result artifact JSON for resumable report generation.
- [done] Browser QA report CLI can parse filled result-template artifacts for resumable report generation.
...
- [done] Browser QA result validation has strict CLI gates for automation handoffs that must fail on stale, duplicate, or mismatched result artifacts.
...
- [done] Browser QA result artifacts can be validated as Markdown or JSON before report handoff, surfacing stale item ids and duplicate result rows.
- [done] Browser QA result validation now flags batch provenance mismatches before resumable screenshot reports are trusted.
